...
let dataEventsUp = data.events.filter(item => item.date > data.currentDate)
...
createCards(dataEventsUp)
form[0].addEventListener('change',(e)=>{
    e.preventDefault()
    createCards(filtrarCategoriasCheckeadas(searchText(inputText.value,dataEventsUp)))
})
